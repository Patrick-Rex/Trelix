...
- 前端通过 `AddViteApp("trelix-client", "../frontend")` 编排，目录相对于 AppHost；使用 `WithNpm(installCommand: "ci")` 安装锁定依赖、`WithReference(server)` 注入服务地址、`WaitFor(server)` 等待 Server 的 HTTPS `/health` 检查通过。
...
- `Trelix:Sample:Enabled` 默认关闭；启用时校验同节的 `ProjectKey`、`EnvironmentKey`、`FileName`、`AccessToken`，注册 `trelix-sample` 并等待 Server 健康检查通过。地址由 Server HTTPS 端点注入，令牌通过 secret 参数传递。先准备已发布文件和应用令牌，再启用示例；详细操作见 [SDK 与示例](../../docs/local-development.md#net-sdk-与示例应用)。
