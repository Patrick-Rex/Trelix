...
发布服务先在事务内执行包含原始并发标记的文件 UPDATE，再读取文件已有最大版本并分配下一版本，插入发布记录、更新指向后提交。回滚使用源历史正文及其草稿修订，不覆盖当前草稿。提交后调用 `IReleaseNotifications` 唤醒文件等待者；通知故障只记录安全诊断，不改变已经提交的成功结果，客户端由后续持久化身份核对发现更新。
...
应用监听入口为 `GET /api/application/configuration/watch`，传入读取接口的三个名称字段及必填的 `knownConfigFileId`、`knownVersion`。发布身份变化返回 200 和 `configFileId`、`version`；正常等待结束且身份未变返回 204，不包含配置正文。400、401、403、404 沿用应用 API 错误契约；容量耗尽返回 503 `listener_capacity_exceeded`。默认等待 30 秒、实例总等待容量 1024，分别由 `Trelix:Distribution:WaitTimeout` 和 `MaxConcurrentListeners` 控制。等待时间大于零且最多五分钟，容量为正整数，启动时校验。认证、资源授权与发布查询均使用独立短数据库作用域，等待前释放；返回前通过数据库重新校验令牌生命周期和范围。
...
| 连接配置复制 | 按当前文件生成、预览和复制本地连接节 | 输入项目与环境业务标识、文件名与发布状态；令牌使用占位符，不读取配置正文或签发令牌 |
...
本地连接配置使用 `Trelix` 节，SDK 按以下字段约定接入；管理界面提供同一节的生成与复制。

| 字段 | 含义 |
| --- | --- |
| `ServerUrl` | 业务应用可访问的 Trelix 服务根地址；管理界面默认使用当前站点地址，允许调整为业务应用可访问的地址 |
| `ProjectKey` | 项目业务标识，保留原始大小写，不使用显示名称或内部 ID |
| `EnvironmentKey` | 项目内环境业务标识，独立于 ASP.NET Core 运行环境 |
| `FileName` | 环境内配置文件名，保留原值 |
| `AccessToken` | 应用只读令牌；复制片段使用 `<APPLICATION_TOKEN>` 占位符，真实值通过 User Secrets 或 `Trelix__AccessToken` 环境变量等外部配置注入 |

复制使用 JSON 序列化保留特殊字符；服务地址只接受不含凭证、查询参数和片段的 HTTP/HTTPS 地址。复制不改变配置内容或发布状态，浏览器剪贴板不可用时保留可手动复制的预览。
...
接入入口为 `await builder.AddTrelixAsync(...)`，适用于 `IHostApplicationBuilder`；先绑定本地节，再应用可选的 `Action<TrelixOptions>`，最后复制并校验参数。SDK 拒绝同一宿主重复注册，固定连接参数仅在重启时更新。同一文件的旧版本不能覆盖当前快照，重复版本不重载；同名重建的新文件 ID 可从版本一重新接入。

SDK 使用独立长生命周期 HTTP 客户端和两分钟连接回收周期，不提前构建宿主服务容器，不继承默认短请求重试管线。默认读取超时 10 秒、监听 HTTP 超时 40 秒；失败采用 1–30 秒的带抖动指数退避。读取响应外壳最多 8 MiB，监听响应最多 16 KiB，配置正文另按 1 MiB 限制校验，覆盖 JSON 字符串转义带来的传输开销。超时与退避设置见 [SDK 说明](../src/sdk/Trelix.Extensions.Configuration/README.md)。

配置源位于默认业务环境变量及命令行源之前，保留本地源的相对顺序；后续自行追加源仍按 .NET 的后添加者优先规则处理。每次源重建创建新的提供程序，共享当前快照与唯一监听循环；最后一个提供程序释放或宿主停止时关闭客户端。JSON 按 .NET 10 的 null、空数组和路径展开语义绑定，业务 Options 校验与资源重新应用由业务应用负责。
