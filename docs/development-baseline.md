...
| 数据存储 | SQLite + EF Core 10 | 单实例与少量管理写入；实体、迁移、时间和版本比较在真实 SQLite 上验证 |
| 管理员认证 | 单个内置管理员 + ASP.NET Core Cookie，无 RBAC | 管理全部项目与环境；安全密码哈希、外部初始化凭证、管理写操作防伪造，见产品方案 |
| 应用访问 | 限定项目与环境的应用只读令牌，使用 Bearer 请求头 | 服务端保存校验摘要，支持到期、撤销、轮换；读取与长轮询均校验授权 |
...
| [global.json](../global.json) | .NET SDK 最低 10.0.300，`rollForward=latestMinor`、`allowPrerelease=false`；Aspire AppHost SDK 13.5.4 与 Visual Studio JavaScript SDK 1.0.6578810 |
